...
ipcMain.handle('GET_GIT_STATUS', async (event, dirPath) => {
  if (!dirPath || dirPath === 'HOME') {
    return { isRepo: false };
  }
  const { exec } = require('child_process');
  const execPromise = (cmd, cwd) => new Promise((resolve) => {
    exec(cmd, { cwd, timeout: 3000 }, (error, stdout, stderr) => {
  ...
  try {
    ...
    const checkRepo = await execPromise('git rev-parse --is-inside-work-tree', dirPath);
    if (checkRepo.error || checkRepo.stdout.trim() !== 'true') {
      return { isRepo: false };
    }

    // Get branch
    const branchRes = await execPromise('git branch --show-current', dirPath);
    const branch = branchRes.stdout.trim();

    // Get toplevel git directory path
    const topLevelRes = await execPromise('git rev-parse --show-toplevel', dirPath);
    const gitRoot = topLevelRes.stdout.trim();

    // Get porcelain git status
    const statusRes = await execPromise('git status --porcelain', dirPath);
    const files = {};
    ...
    if (!statusRes.error && statusRes.stdout) {
      ...
      for (const line of lines) {
        ...
        // If path is wrapped in quotes, unwrap it
